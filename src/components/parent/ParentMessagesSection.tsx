...
export default function ParentMessagesSection() {
  const t = useTranslations("ParentMessagesSection");
  ...
  // Fetch journal data to extract teacher info for new conversations
  ...
  }>(newDialogOpen ? "/api/parent/journal" : null, fetcher);
  ...
  // Extract unique teachers from journal entries
  const availableTeachers: TeacherOption[] = [];
  if (journalData?.entries) {
    const seen = new Set<string>();
    for (const entry of journalData.entries) {
      if (entry.teacher && !seen.has(entry.teacher.id)) {
  ...
  const handleCreateConversation = async () => {
    if (!newSubject.trim() || !selectedTeacherId || creating) return;
    setCreating(true);
    try {
      const res = await fetch("/api/conversations", {
        ...
        body: JSON.stringify({
          ...
          participantIds: [selectedTeacherId],
        }),
      });
        ...
        <DialogContent>
          ...
          <div className="space-y-4 py-2">
            <div className="space-y-2">
              ...
              {availableTeachers.length > 0 ? (
                ...
                <p className="text-sm text-gray-500">{t("noTeachersFound")}</p>
              )}
            </div>
            <div className="space-y-2">
          ...
          <DialogFooter>
            ...
            <Button
              ...
              disabled={!newSubject.trim() || !selectedTeacherId || creating}
            >
              {t("create")}
            </Button>
          </DialogFooter>
