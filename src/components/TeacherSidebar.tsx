...
import {
  ...
  TeacherSidebarGroup,
} from "@/config/teacherSidebarConfig";
import {
  Home,
  LayoutGrid,
  LayoutDashboard,
...
/* ------------------------------------------------------------------ */
const STORAGE_KEY = "teacherSidebarGroups";
...
export default function TeacherSidebar({
  ...
  const activeGroupId = useMemo(() => {
    for (const group of TEACHER_SIDEBAR_GROUPS) {
  ...
  /* --- Render helpers ---------------------------------------------- */
  const HomeIcon = resolveIcon(TEACHER_HOME_ITEM.iconName);
  const isHomeActive = activeSection === TEACHER_HOME_ITEM.key;
  ...
  return (
    <nav className="space-y-1">
      ...
      <button
        type="button"
        onClick={() => setActiveSectionAction(TEACHER_HOME_ITEM.key)}
      ...
      >
        ...
        <span>{t(TEACHER_HOME_ITEM.labelKey)}</span>
      </button>

      {/* Scrollable group list */}
      <div className="flex-1 overflow-y-auto">
        {TEACHER_SIDEBAR_GROUPS.map((group) => (
          <SidebarGroup
            key={group.id}
            group={group}
