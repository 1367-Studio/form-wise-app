...
import {
  ...
  ParentSidebarGroup,
} from "@/config/parentSidebarConfig";
import {
  Sheet,
  SheetContent,
  SheetTrigger,
...
/*  Component                                                          */
/* ------------------------------------------------------------------ */
export default function ParentMobileSidebar({
  activeSection,
  ...
  const activeGroupId = useMemo(() => {
    for (const group of PARENT_SIDEBAR_GROUPS) {
  ...
  /* --- Render ------------------------------------------------------ */
  const HomeIcon = resolveIcon(PARENT_HOME_ITEM.iconName);
  const isHomeActive = activeSection === PARENT_HOME_ITEM.key;
          ...
          <div className="flex h-full flex-col">
            ...
            <div className="flex-1 overflow-y-auto pr-1">
              <nav className="space-y-1">
                ...
                <button
                  type="button"
                  onClick={() => handleSelectSection(PARENT_HOME_ITEM.key)}
                ...
                >
                  ...
                  <span>{t(PARENT_HOME_ITEM.labelKey)}</span>
                </button>

                {/* Groups */}
                {PARENT_SIDEBAR_GROUPS.map((group) => (
                  <MobileGroup
                    key={group.id}
                    group={group}
