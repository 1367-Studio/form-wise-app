...
import {
  ...
  StaffSidebarGroup,
} from "@/config/staffSidebarConfig";
import {
  Home,
  Users,
  ClipboardCheck,
...
/* ------------------------------------------------------------------ */
const STORAGE_KEY = "staffSidebarGroups";
...
export default function StaffSidebar({
  ...
  const activeGroupId = useMemo(() => {
    for (const group of STAFF_SIDEBAR_GROUPS) {
  ...
  /* --- Render helpers ---------------------------------------------- */
  const HomeIcon = resolveIcon(STAFF_HOME_ITEM.iconName);
  const isHomeActive = activeSection === STAFF_HOME_ITEM.key;
  ...
  return (
    <nav className="space-y-1">
      ...
      <button
        type="button"
        onClick={() => setActiveSectionAction(STAFF_HOME_ITEM.key)}
      ...
      >
        ...
        <span>{t(STAFF_HOME_ITEM.labelKey)}</span>
      </button>
